...
import { runContactAcquisitionImport } from "@/lib/contact-acquisition/importer";

export const dynamic = "force-dynamic";

function streamImport(triggeredBy, signal) {
  ...
  return new Response(
    new ReadableStream({
      async start(controller) {
        ...
        try {
          const result = await runContactAcquisitionImport({
            triggeredBy,
            onProgress: write,
            signal,
          });
...
export async function POST(request) {
  ...
  if (body.stream === true) {
    return streamImport(admin.user.id, request.signal);
  }
