...
function applyContactFilters(query, { source = "", hasPhone = false, hasLinkedin = false } = {}) {
  let nextQuery = query;

  const normalizedSource = String(source || "").trim();
  if (normalizedSource) {
    nextQuery = nextQuery.eq("source", normalizedSource);
  }

  if (hasPhone) {
    nextQuery = nextQuery.not("phone", "is", null).neq("phone", "");
  }

  if (hasLinkedin) {
    nextQuery = nextQuery.not("linkedin_url", "is", null).neq("linkedin_url", "");
  }

  return nextQuery;
}
export async function fetchContactQueuePage({
  page = 1,
  pageSize = 25,
  search = "",
  source = "",
...
export async function fetchContactSources() {
  const supabase = await createSupabaseServerClient();
